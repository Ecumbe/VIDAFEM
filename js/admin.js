...
function loadPromoStatus() {
    ...
    fetch(API_URL, { method: "POST", body: JSON.stringify({ action: "get_promo_list" }) })
    .then(r => r.json())
    .then(res => {
        container.innerHTML = "";
...
window.deletePromo = function(id) {
    ...
    fetch(API_URL, { method: "POST", body: JSON.stringify({ action: "delete_promotion", id: id }) })
    .then(r => r.json())
    .then(res => {
...
if(formPromo) {
    formPromo.addEventListener('submit', (e) => {
        ...
        .finally(() => { btn.disabled = false; btn.innerText = "Publicar / Actualizar"; });
    });
}