...
function setupAppointmentListeners() {
    ...
    if(formAppt) {
        ...
        newForm.addEventListener('submit', function(e) {
            ...
            .then(res => {
                if (res.success) {
                    // 1. Cerrar el modal de formulario
                    ...
                    if (telPac && telPac.length >= 9) {
                        // Formatear numero (quitar 0 inicial si tiene, agregar 593)
                        let cleanTel = telPac.replace(/^0+/, '');
                        btnWa.href = `https://wa.me/593${cleanTel}?text=${encodeURIComponent(mensaje)}`;
                        btnWa.style.display = "flex"; // Mostrar si hay telefono
                    } else {
                        btnWa.style.display = "none"; // Ocultar si no hay telefono
                    }

                    // 3. ABRIR EL MODAL DE ÉXITO
